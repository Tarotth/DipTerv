...
public:
	...
	void setupUi(QMainWindow *kinect_appClass)
	{
		...
		kinect_appClass->insertToolBarBreak(mainToolBar);
...
#ifdef _DEBUG
		ogreRoot->loadPlugin("RenderSystem_Direct3D11_d");
#else
		ogreRoot->loadPlugin("RenderSystem_Direct3D11");
#endif
		...
		//ogreWidget_ = new qt_ogre::QOgreWidget(ogreRoot, splitter);
		ogreWidget_ = QOgreWidgetv2::getInstance(ogreRoot, this, splitter);
		...
		//Ogre::SceneNode* rootNode = ogreWidget_->getSceneManager()->getRootSceneNode();
		////Ogre::SceneNode* archwayNode = rootNode->createChildSceneNode();
		//Ogre::SceneNode* sphereNode = rootNode->createChildSceneNode();
		//Ogre::Entity* mEntity = ogreWidget_->getSceneManager()->createEntity("mySphere", Ogre::SceneManager::PT_SPHERE);
		//sphereNode->attachObject(mEntity);
		//mEntity->setMaterialName("Shader1_HLSL_4_0");
		//sphereNode->setPosition(Ogre::Vector3(0, 0, 0));
		//sphereNode->setScale(Ogre::Vector3(0.5, 0.5, 0.5));
		//Ogre::Entity* archway = ogreWidget_->getSceneManager()->createEntity("archway", "Archway.mesh");
		//archwayNode->attachObject(archway);
	...
	void onZoomEvent(QWheelEvent *event) {
		...
		if (scroll.y() > 0)
			ogreWidget_->getSceneManager()->getCamera("Main Camera")->moveRelative(Ogre::Vector3(0, 0, -10.0));
		if (event->angleDelta().y() < 0)
			ogreWidget_->getSceneManager()->getCamera("Main Camera")->moveRelative(Ogre::Vector3(0, 0, 10));
	}
