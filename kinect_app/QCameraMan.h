...
namespace OgreBites
{
	...
	public:
		...
		virtual void cameraShift(int dir)
		{
			const Ogre::Quaternion &camRot = mCamera->getDerivedOrientation();
			Ogre::Vector3 xyzTranslation;
			if(dir > 0)
				xyzTranslation = Ogre::Vector3(1.0f, 0.0f, 0.0f); //Move left, down, forward
			else if (dir < 0)
				xyzTranslation = Ogre::Vector3(-1.0f, 0.0f, 0.0f);
			else if (dir = 0)
				xyzTranslation = Ogre::Vector3(0.0f, 0.0f, 0.0f);
			mCamera->setPosition(mCamera->getPosition() + camRot * xyzTranslation);
		}
		...
		virtual void setCameraPos(Ogre::Vector3 vec3)
		{
			mCamera->setPosition(vec3);
		}

		virtual Ogre::Vector3 getCameraPos()
		{
